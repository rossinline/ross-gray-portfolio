        ...
        <div v-for="project in projects" :key="project.title" class="relative">
          <button @click="handleButtonClick(project)"
            ...
            <div class="flex items-center">
              <component :is="getIconComponent(project.icon)" class="w-16 h-16 sm:w-6 sm:h-6 md:w-10 md:h-10 mr-4" />
          ...
          <div class="w-full md:w-2/3 md:pr-8">
            ...
            <div class="py-2">
              ...
              <div class="flex flex-wrap items-center md:p-0">
                <img v-if="selectedProject.image_screenshot" v-for="(image, index) in getImageUrls(selectedProject.image_screenshot)" :key="index" :src="image" 
                  alt="Feedback Screenshot"
                  class="w-10 h-auto mr-4 rounded-[8px] border border-lm-text dark:border-dm-text" />
          ...
          <div class="w-full md:w-1/3 md:pl-5 md:mt-5 mt-4">
            <div class="flex flex-wrap justify-center gap-5">
              <img v-if="selectedProject.image_screenshot && selectedProject.image_screenshot.trim() !== ''"
                v-for="(image, index) in getImageUrls(selectedProject.image_screenshot)" :key="index" :src="image"
                alt="Project Screenshot"
                class="w-32 h-auto md:w-64 mr-0 md:mr-4 rounded-[8px] border border-lm-text dark:border-dm-text" />
            </div>
          </div>
        </div>
        <p class="flex items-center text-sm py-8 text-gray-500 dark:text-gray-300">
          <Info class="w-5 h-5 mr-2" />Projects Section created with backend Laravel and Axios API
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getImageUrls(images) {
      return images.split(',').map(image => new URL(image.trim(), import.meta.url).href);
    },
  },
};
</script>
