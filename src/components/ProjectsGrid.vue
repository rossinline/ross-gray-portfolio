        ...
        <div class="flex flex-wrap">
          <div class="w-full md:w-2/3 md:pr-8">
            ...
            <div class="py-2">
              ...
              <div class="flex flex-wrap items-center md:p-0">
                <img v-if="selectedProject.feedback_logo && selectedProject.feedback_logo.trim() !== ''" v-for="(image, index) in getImageUrls(selectedProject.feedback_logo)" :key="index" :src="image" alt="Feedback Screenshot" class="w-10 h-auto mr-4 rounded-[8px] border border-lm-text dark:border-dm-text" />
          ...
          <div class="w-full md:w-1/3 md:pl-5 md:mt-5 mt-4">
            <div class="flex flex-wrap justify-center gap-5">
              <img v-if="selectedProject.image_screenshot && selectedProject.image_screenshot.trim() !== ''" v-for="(image, index) in getImageUrls(selectedProject.image_screenshot)" :key="index" :src="image" alt="Project Screenshot" class="w-32 h-auto md:w-64 mr-0 md:mr-4 rounded-[8px] border border-lm-text dark:border-dm-text" />
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getImageUrls(images) {
      // Map each image path to a resolved URL
